...
class LinkProcessor:
    def __init__(self, config_manager: Any = None, file_resolver: Any = None, logger: Any = None) -> None:
    ...
    def _load_policy_from_config(self) -> Dict[str, Any]:
        ...
        if not self.config_manager:
            return {"check_exists": False}
            
        try:
            # 尝试从配置管理器获取链接处理配置
            if hasattr(self.config_manager, 'get_config'):
                try:
                    config = self.config_manager.get_config("link_processing", {}, "core")
                except Exception:
                    # 如果get_config需要参数但调用失败，尝试无参数调用
                    try:
            ...
            policy = {
                ...
                "check_exists": link_config.get("relative_paths", True),
                "external_links": link_config.get("external_links", True),
                "image_links": link_config.get("image_links", True),
                "mermaid_links": link_config.get("mermaid_links", True),
                "file_protocol": link_config.get("file_protocol", True),
                "cache_enabled": link_config.get("cache_enabled", True),
                "cache_size": link_config.get("cache_size", 1000),
                "error_handling": link_config.get("error_handling", "strict"),
                "windows_specific": link_config.get("windows_specific", {}),
                "security": link_config.get("security", {}),
                "logging": link_config.get("logging", {})
            }
    ...
    def process_link(self, ctx: LinkContext) -> LinkResult:  # pragma: no cover - 简化骨架
        try:
            ...
            if link_type in (LinkType.RELATIVE_MD, LinkType.DIRECTORY):
                ...
                # 仅在明确配置时执行URL校验（避免默认fail-closed影响已有用例）
                should_validate_url = False
                sec = (self.policy.get("security") or {})
                if sec.get("allowed_protocols") or sec.get("allowed_domains") is not None:
                    should_validate_url = True
            ...
            return LinkResult(success=False, action="", payload={}, message=str(ex), error_code=ErrorCode.INTERNAL_ERROR)
