...
class EnhancedErrorHandler:
    ...
    def _load_error_strategy(self):
        ...
        if self.config_manager:
            try:
                error_config = self.config_manager.get_unified_config("error_handling", {})
                self.error_strategy = error_config.get("strategy", "graceful")
                self.logger.info(f"加载错误处理策略: {self.error_strategy}")
    ...
    def handle_error(self, exception: Exception, context: Optional[Dict[str, Any]] = None,
        ...
        try:
            ...
            if handler:
                # 使用注册的处理器
                error_info = handler(exception, error_context)
            else:
                # 使用默认处理器
            ...
            # 创建基本错误信息
            return self._create_basic_error_info(exception, str(e))
