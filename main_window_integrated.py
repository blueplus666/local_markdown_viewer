...
def setup_logging():
    ...
    if not _test_mode:
        _handlers.insert(0, logging.FileHandler(logs_dir / "integrated.log", encoding='utf-8'))
    logging.basicConfig(
        level=logging.INFO,
...
class IntegratedMainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # 启动期动态导入状态占位
        self._startup_import_status: Optional[Dict[str, Any]] = None
        ...
        # 在状态栏反馈动态导入状态
        self._reflect_startup_import_status()
    ...
    def _init_core_components(self):
        ...
        try:
            ...
            # 启动期动态导入状态检查（便于启动时直观看到）
            try:
                importer = DynamicModuleImporter(self.config_manager)
                result = importer.import_module('markdown_processor', ['markdown'])
                self._startup_import_status = result
                if result.get('success'):
                    if result.get('module') == 'markdown':
                        self.logger.info("动态导入未命中，使用备用markdown库 (fallback)")
                    else:
                        self.logger.info(f"动态导入成功: module={result.get('module')} path={result.get('path')}")
                else:
                    self.logger.warning(f"动态导入失败: {result.get('error_code')} - {result.get('message')}")
            except Exception as e:
                self._startup_import_status = {'success': False, 'error_code': 'UNKNOWN_ERROR', 'message': str(e)}
                self.logger.error(f"动态导入状态检查异常: {e}")
    ...
    def _init_window(self):
        ...
        # 设置窗口居中显示
        self._center_window()
        # 记录几何信息便于与 main.py 比对
        try:
            from PyQt5.QtGui import QGuiApplication
            scr = QGuiApplication.screenAt(self.pos()) or QGuiApplication.primaryScreen()
            ag = scr.availableGeometry() if scr else QApplication.desktop().availableGeometry(self)
            fg = self.frameGeometry(); g = self.geometry()
            dpr = getattr(self.windowHandle(), 'devicePixelRatio', lambda: 1.0)()
            self.logger.info(
                f"GEOM|integrated.init|screen.available={ag.x()},{ag.y()},{ag.width()},{ag.height()} "
                f"win.geom={g.x()},{g.y()},{g.width()},{g.height()} win.frame={fg.x()},{fg.y()},{fg.width()},{fg.height()} "
                f"dpr={dpr}"
            )
        except Exception:
            pass
    ...
    def _reflect_startup_import_status(self):
        """在状态栏/弹窗中反映启动期动态导入状态"""
        try:
            st = self._startup_import_status or {}
            if not st:
                return
            if st.get('success'):
                if st.get('module') == 'markdown':
                    self.statusBar().showMessage("动态导入未命中，使用备用markdown库")
                else:
                    path = st.get('path') or ""
                    self.statusBar().showMessage(f"动态导入成功: {st.get('module')} {path}")
            else:
                msg = f"动态导入失败: {st.get('error_code')} - {st.get('message')}"
                self.statusBar().showMessage(msg)
                # 同时弹窗提示一次（非致命）
                QMessageBox.warning(self, "动态导入失败", msg)
        except Exception as e:
            self.logger.error(f"启动期状态反映异常: {e}")
